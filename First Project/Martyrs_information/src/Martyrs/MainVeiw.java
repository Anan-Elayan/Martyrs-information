...
public class MainVeiw extends Pane {
	...
	public MainVeiw() {
		...
		vbox.getChildren().addAll(btnLocation, btnStatistic, btnSave);
		...
		btnStatistic.setOnAction(e -> {
			...
			NodeDoubleLinkedList loc = GUI.data.search(LocationPane.txtLocation.getText());
			if (loc == null) { // not exit location
				new Warning(LocationPane.txtLocation.getText() + " not exists");
				return;
			}
			if(loc.getListMartyrs().size()==0) { // no record in this location
				new Warning(LocationPane.txtLocation.getText() + " does not have any recorde");
				return;
			}
